...
public class Solution2_4 {
    ...
    해당 시점에 공급 가능한 밀가루 수량(supplies), 원래 공장으로부터 공급받을 수 있는 시점 k
    가 주어질 때, 밀가루가 떨어지지 않고 공장을 운영하기 위해서
    최소한 몇 번 해외 공장으로부터 밀가루를 공급받아야 하는지를 return 하도록
    solution 함수를 완성하세요.
    ...
    - dates에 들어있는 날짜에 공급되는 밀가루는 작업 시작 전 새벽에 공급되는 것을 기준
...
*/
    public int solution(int stock, int[] dates, int[] supplies, int k) {
        ...
        PriorityQueue<Integer> reSupplies = new PriorityQueue<>(
                Collections.reverseOrder());

        // 재고량이 원래 공장에서 공급받을 수 있는 날보다 작을때만 해외에서 공급받기
        for (int day = 0; day < k; day++) {
            if (index < dates.length && dates[index] == day) {
                reSupplies.add(supplies[index]);
                index++;
            }

            if (stock == 0) {
                stock += reSupplies.poll();
                answer++;
            }
            stock--;
        }
